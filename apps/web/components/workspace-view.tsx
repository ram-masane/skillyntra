...
type Validation = { id: number; skill: string; score: number; status: string };
...
export default function WorkspaceView({ role, section }: { role: string; section: string }) {
  ...
  if (section === "my-skills") return <section className="workspace-view"><div className="section-banner"><CheckCircle2 size={20} /><span>Industry validation is issued after a passing company assessment.</span></div>{error && <div className="error-box">{error}</div>}{validations.length === 0 && !error && <div className="empty-box"><strong>No validated skills yet</strong><span>Open a course assessment, pass it, and your verified skills will appear here.</span></div>}<div className="validation-grid">{validations.map((item) => <article className="validation-card" key={`${item.skill}-${item.id}`}><CheckCircle2 size={20} /><div><strong>{item.skill}</strong><span>{item.status} · score {item.score}%</span></div></article>)}</div></section>;
