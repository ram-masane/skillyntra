...
export default function PartnerConsoleView({ assessment = false }: { assessment?: boolean }) {
  ...
  if (assessment) return <section className="builder-view"><div className="builder-card"><div className="panel-kicker">Assessment authoring</div><h2>Create a company skill check</h2><p>Define the assessment connected to a published course and its evaluated skills.</p><label>Exam title<input value={assessmentTitle} onChange={(event) => setAssessmentTitle(event.target.value)} placeholder="e.g. Data Analytics Partner Skill Check" /></label><label>Course<select value={courseId} onChange={(event) => setCourseId(Number(event.target.value))}><option value="2">Data Analytics</option><option value="1">Industrial IoT</option><option value="4">Predictive Maintenance</option></select></label><div className="builder-row"><label>Duration<input type="number" defaultValue="30" /></label><label>Passing score<input type="number" value={passingScore} onChange={(event) => setPassingScore(Number(event.target.value))} /></label></div><label>Skills evaluated<input value={assessmentSkills} onChange={(event) => setAssessmentSkills(event.target.value)} placeholder="Python, SQL, Data Analysis" /></label><label>Question<input value={questionText} onChange={(event) => setQuestionText(event.target.value)} placeholder="e.g. Which library is used for tabular data?" /></label><label>Options<input value={questionOptions} onChange={(event) => setQuestionOptions(event.target.value)} placeholder="Option A, Option B, Option C, Option D" /></label><div className="builder-row"><label>Correct answer<input value={correctAnswer} onChange={(event) => setCorrectAnswer(event.target.value)} /></label><label>Question skill<input value={questionSkill} onChange={(event) => setQuestionSkill(event.target.value)} placeholder="Python" /></label></div><button onClick={saveAssessment}>{published ? "Assessment published" : "Publish assessment"}</button>{published && <div className="success-box">Assessment published and attached to the selected course.</div>}</div></section>;
