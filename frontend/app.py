...
from backend.services.course_service import course_health
...
def course_result(name):
    ...
    top = demand_of(jobs).head(15).skill.tolist()
    tn = {x.lower() for x in taught}
    covered = [x for x in top if x.lower() in tn]
    missing = [x for x in top if x.lower() not in tn]
    align = round(100 * len(covered) / max(1, len(top)))
    placement = float(r.placement_rate)
    emp = float(r.employer_validation)
    health = round(.45 * align + .30 * placement + .25 * emp)
    return taught, placement, align, health, missing


def cap_metrics(df):
    x = df.copy()
    x["gap"] = x.estimated_demand - x.annual_capacity
    x["status"] = x.gap.apply(lambda v: "Potential shortage" if v > 0 else "Within capacity")
...
# ---------------- GOVERNMENT ----------------
    ...
    if not x.empty:
        ...
        for _,r in x.head(4).iterrows():st.markdown(f'<div class="card" style="margin:8px 0"><b>{r.skill}</b> — {"Increase / prioritize training capacity" if r.gap>0 else "Review for possible oversupply"}.</div>',unsafe_allow_html=True)

# ---------------- EVIDENCE ----------------
else:
    demand=demand_of(jobs)
    emap=dict(zip(employer.skill,employer.employer_validation));emap.update(st.session_state.overrides)
    ev=demand.copy();ev["employer_validation"]=ev.skill.map(emap).fillna(50)
    conf=dict(zip(employer.skill,employer.confidence));ev["confidence"]=ev.skill.map(conf).fillna("Medium")
    ev["evidence_score"]=(ev.job_signals.rank(pct=True)*45+ev.employer_validation/100*35+ev.confidence.map({"High":20,"Medium":12,"Low":7}).fillna(5)).round(1)
    ...
    st.markdown('<div class="notice" style="margin-top:18px">Production path: verified partner companies → authenticated course/assessment workspace → assessment results → employer validation → refreshed SkillOra intelligence.</div>',unsafe_allow_html=True)
